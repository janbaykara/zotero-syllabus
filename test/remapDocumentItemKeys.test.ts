...
function incomingNoteFixture() {
  ...
  return {
    remoteDoc,
    emptyDoc: emptyCollectionDocument(),
    cached: {
      noteId: 10,
      noteVersion: 5,
      document: cachedDoc,
...
describe("syllabusNote remapping", function () {
  ...
  describe("selectItemKeyRemapForDocument", function () {
    ...
    it("applies remaps only to notes in the merge library", function () {
      ...
      assert.isNull(
        selectItemKeyRemapForDocument("2:ABC12345", 1, keys, keyMap),
      );
      assert.isNull(
  ...
  describe("shouldAdoptIncomingNote", function () {
    it("ignores an echo of the version already in cache", function () {
      const { cached, remoteDoc } = incomingNoteFixture();
      assert.equal(
        shouldAdoptIncomingNote({
          writeInFlight: true,
          cached,
    ...
    it("does not bump version on an empty payload during a write", function () {
      const { cached, emptyDoc } = incomingNoteFixture();
      assert.equal(
        shouldAdoptIncomingNote({
          writeInFlight: true,
          cached,
