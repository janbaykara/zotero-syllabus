...
enable-subcollections-message =
    ...
    • 授業ごとにフォルダが作成または流用され、シラバスに合わせて名前が付けられます（例：「授業 1: タイトル」）。
...
page-drop-import-file = .syllabus ファイルをドロップして読み込む
...
settings-subcollections-desc = 既定ではオフです。オンにすると、各授業がこのコレクション配下にフォルダを持ちます。フォルダはシラバスに合わせて作成・改名・削除されます。既存の子コレクションも削除されることがあります。オフにしてもフォルダは残ります。
