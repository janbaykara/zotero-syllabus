...
5. Do not rename stored identifiers: note title `"Syllabus"`, collection `"Reading Schedule"`, child folder `"Pinned"`, playground `"Syllabus Tour"`, plugin JSON heading, tags `zotero-syllabus`, `pinned`, `zotero-syllabus-pinned-intention`, `zotero-syllabus-pinned-collection`, `zotero-syllabus-gallery:{collectionKey}`.
