...
export function buildSyllabusClassGroups(
  ...
  classGroups: SyllabusClassGroup[];
  ...
  const sortedFinalClassNumbers = Array.from(finalClassNumbers).sort((a, b) => {
    if (a === null && b === null) return 0;
    if (a === null) return -1;
    if (b === null) return 1;
    return a - b;
...
export function useSyllabusClassGroups(
  ...
  return useMemo(
    () =>
      buildSyllabusClassGroups(collectionId, syllabusItems, syllabusMetadata),
